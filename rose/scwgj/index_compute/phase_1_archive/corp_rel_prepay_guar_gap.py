...
@register(name="corp_rel_prepay_guar_gap",
          persistent="parquet",
          ...
          dependencies=["target_company_v",
                        ...
                        "pboc_corp_pay_jn_v"])
def corp_rel_prepay_guar_gap(entry: Entry, param: Parameter, logger):

    # 预付与关联方内保外贷到期的时间间隔
    sql_str = """
                    select company_name, 
                        corp_code, 
                        min(tmp_gap) as corp_rel_prepay_guar_gap
                    from (
                        select a.bbd_company_name as company_name, 
                            a.corp_code, 
                            abs(datediff(contractdate, pay_date)) as tmp_gap
