...
@register(name="corp_same_payrcv_cp",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v",
                        "pboc_corp_pay_jn_v",
                        "pboc_corp_rcv_jn_v"])
def corp_same_payrcv_cp(entry: Entry, param: Parameter, logger):
    # 货物贸易收付对方交易对手名称是否为同一境外对手
    ...
    tmp1_sql = f"""
                    select a.bbd_company_name as company_name, a.corp_code, cp_rcver_names
    ...
    sql_str = """
                select company_name, 
                    corp_code, 
                    get_intersection(cp_rcver_names, cp_payer_names) as corp_same_payrcv_cp
                from tmp
              """
