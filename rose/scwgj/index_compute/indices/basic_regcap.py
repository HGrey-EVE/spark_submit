...
@register(name="basic_regcap",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v",
                        "qyxx_basic_v"])
def basic_regcap(entry: Entry, param: Parameter, logger):

    # 注册资本
    sql_str = """
                select a.bbd_company_name as company_name, 
                    a.corp_code, 
                    b.regcap_amount as basic_regcap
                from target_company_v a
                join (
                    select company_name, regcap_amount, bbd_qyxx_id
                    from qyxx_basic_v
                )b
