...
@register(name="corp_servpay_genrcv_prop",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v", "pboc_corp_pay_jn_v", "pboc_corp_rcv_jn_v"])
def corp_servpay_genrcv_prop(entry: Entry, param: Parameter, logger):
    ...
    # 【对公付款】服贸项下付汇 【指标计算当天过去一年】
    sql = f'''
        select corp_code, sum(tx_amt_usd) as c12
          from (select corp_code, tx_amt_usd
    ...
    sql = """
        select 
            ...
            ,case when v1.c12 is null and v2.c1 is null then null else coalesce(v1.c12, 1)/coalesce(v2.c1, 1) end corp_servpay_genrcv_prop
