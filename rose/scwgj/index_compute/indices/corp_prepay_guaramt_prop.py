...
@register(name="corp_prepay_guaramt_prop",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v",
                        "pboc_corp_pay_jn_v",
                        "pboc_con_exguaran_new_v"])
def corp_prepay_guaramt_prop(entry: Entry, param: Parameter, logger):
    ...
    part2_sql = f"""
                    ...
                    from (
                        select corp_code, 
                            usdguaranamount, 
                            concat(substring(maturity, 1, 7), '-01') as new_maturity
                    ...
                    group by corp_code, maturity_tmp
                """
    ...
    sql_str = """
                select company_name, 
                    corp_code, 
                    count(1) as corp_prepay_guaramt_prop
                from (
                    select company_name, 
                        corp_code, 
                        tmp_prop 
