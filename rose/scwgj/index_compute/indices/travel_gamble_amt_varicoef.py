...
@register(name="travel_gamble_amt_varicoef",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v",
                        "pboc_corp_pay_jn_v"])
def travel_gamble_amt_varicoef(entry: Entry, param: Parameter, logger):
    t_date_dict = param.get('tmp_date_dict')
    t_date = t_date_dict.get('pboc_corp_pay_jn')

    # 服贸（旅游）向周边涉赌国家付款金额变异系数 分子标准差 【指标计算当天过去一年】
    c39_sql = f"""
                select corp_code, 
    ...
    # 服贸（旅游）向周边涉赌国家付款金额变异系数  分母平均值 【指标计算当天过去一年】
    c40_sql = f"""
                 select corp_code, 
    ...
    sql_str = """
                select company.corp_code, 
                    ...
                    end travel_gamble_amt_varicoef
