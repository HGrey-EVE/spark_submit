...
@register(name="import_higher_decleared",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["pboc_impf_v",
                        "target_company_v"])
def import_higher_decleared(entry: Entry, param: Parameter, logger):
    ...
    sql =  f'''
        select a.bbd_company_name as company_name, a.corp_code,
               case when b.corp_code is not null then 1
                    else 0 end import_higher_decleared
                                    ...
                                    select corp_code, merch_code, unit_code, avg(prod_deal_amt_usd) as company_avg
                                      from (
                                            select corp_code, merch_code, unit_code, prod_deal_amt_usd
                                              from pboc_impf_v
                                             where imp_date between add_months('{pboc_impf_end_date}', -12) and date('{pboc_impf_end_date}')
                                            )
