...
@register(name="service_notravel_freq_pay",
          persistent="parquet",
          persistent_param={"mode": "overwrite"},
          dependencies=["target_company_v", "pboc_corp_pay_jn_v"])
def service_notravel_freq_pay(entry: Entry, param: Parameter, logger):
    t_date_dict = param.get('tmp_date_dict')
    t_date = t_date_dict.get('pboc_corp_pay_jn')
    # 服贸（非旅游）向周边涉赌国家高频付款
    sql = f'''
        select company.corp_code, company.bbd_company_name, service_notravel_freq_pay_value, 
        case when service_notravel_freq_pay_value is null then null
             when service_notravel_freq_pay_value > 10 then 1 
            else 0
            end service_notravel_freq_pay 
        from target_company_v company
        left join
            (select corp_code, count(*) as service_notravel_freq_pay_value, '' as service_notravel_freq_pay
              from (
                    select *
