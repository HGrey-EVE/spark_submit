...
<<<<<<< HEAD
=======
            self.logger.info(f"load {self.table_name} to data share success")
>>>>>>> d0e4a9fd6031d5de7af53fc728564ca3ec06410b
...
=======
        ...
        if self.need_write_result:
            if not table_df:
                raise FriendlyException(f'{self.table_name} table df is null!')
            table_df.write.parquet(self.result_path, mode='overwrite')

        if not self.need_write_result:
            if not check_hdfs_path_exists(self.result_path):
                raise FriendlyException(f'path {self.result_path} not exists!')
